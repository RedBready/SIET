    ...
    while True:
        ...
        if child_pid == 0:
            ...
            if sReq == 'get':
                ...
                j = 1
                while data:
                    sUdp = bytes.fromhex('0003' + format(j, '04x')) + data
                    ConnDATA.sendto(sUdp, (raddress, rport))
                    try:
                        buffer, (raddress, rport) = ConnDATA.recvfrom(MAX_BLKSIZE)
                    except socket.error:
                        ...
                        break
                    nBlock = int.from_bytes(buffer[2:4], 'big')
                    if buffer[1] != 4 or nBlock != j:
                        print('[ERR]:[' + raddress + ']:[' + sReq + '] answer packet not valid:', buffer[1], nBlock, j)
                        break
